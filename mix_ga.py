...
def start(inp: WusnInput, pop_size=30, max_iters=10000, patience=100,
          cross_rate=0.8, mut_rate=0.1, km_ratio=0.5, eco_sys=EcoSys.get_instance()):
    ...
    toolbox.register('initialize1', init.kmeans_greedy_init)
    toolbox.register('initialize2', init.random_init)
    ...
    km_size = int(km_ratio * pop_size)
    population1 = toolbox.initialize1(inp, km_size)
    population2 = toolbox.initialize2(inp, pop_size - km_size)
...
def parse_arguments():
    ...
    parser.add_argument('-o', '--output', default='out/mix-ga/')
    ...
    parser.add_argument('--km-ratio', type=float, default=0.5)
...
if __name__ == '__main__':
    ...
    out, logs = start(inp, pop_size=args.pop_size, max_iters=args.iters,
                      ...
                      mut_rate=args.mut_rate, km_ratio=args.km_ratio, eco_sys=ec)
